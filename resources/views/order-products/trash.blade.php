@extends('layouts.app', ['page' => 'order-products-trash'])
...
@section('main')
    <div class="main__conent-box styled-box">
        ...
        @include('tables.trash-template', ['tableName' => 'order-products'])
    </div>

    @can('delete-from-trash')
        <x-modals.multiple-delete action="{{ route('order.products.destroy') }}" :force-delete="true" />
    @endcan

    @can('edit-orders')
        <x-modals.multiple-restore action="{{ route('order.products.restore') }}" />
    @endcan

    <x-modals.edit-table-columns action="{{ route('settings.update-table-columns') }}" table="order-products" :columns="$allTableColumns" />
@endsection
